Theme.register('hackerman', {
    mode: 'dark',
    backgrounds: [
        './src/themes/hackerman/backgrounds/1-synth-scape.jpg',
        './src/themes/hackerman/backgrounds/2-geometric.webp',
        './src/themes/hackerman/backgrounds/omarchy.webp',
    ],
    colors: {
        accent: '#82FB9C',
        selection: '#1f253a',
        muted: '#2d3450',
        background: '#0B0C16',
        dark_background: '#080910',
        darker_background: '#06060c',
        lighter_background: '#151828',
        foreground: '#ddf7ff',
        dark_foreground: '#6a6e95',
        light_foreground: '#b5c5db',
        bright_foreground: '#ddf7ff',
        red: '#50f872',
        yellow: '#50f7d4',
        orange: '#50f7a3',
        green: '#4fe88f',
        cyan: '#7cf8f7',
        blue: '#829dd4',
        magenta: '#86a7df',
        brown: '#287b51',
        bright_red: '#85ff9d',
        bright_yellow: '#a4ffec',
        bright_green: '#9cf7c2',
        bright_cyan: '#d1fffe',
        bright_blue: '#c4d2ed',
        bright_magenta: '#cddbf4',
        hyprland_active_border: 'rgba(26a269ee) rgba(2ec27eee) 45deg',
    },
})
